export const GET_ALL_PRIZES = 'GET_ALL_PRIZES';
export const GET_ALL_PRIZES_SUCCESS = 'GET_ALL_PRIZES_SUCCESS';
export const GET_ALL_PRIZES_ERROR = 'GET_ALL_PRIZES_ERROR';
export const GET_PRIZE = 'GET_PRIZE';
export const GET_PRIZE_SUCCESS = 'GET_PRIZE_SUCCESS';
export const GET_PRIZE_ERROR = 'GET_PRIZE_ERROR';

export const CHANGE_PRIZE_PAGE = 'CHANGE_PRIZE_PAGE';
export const CHANGE_PRIZE_PAGE_SUCCESS = 'CHANGE_PRIZE_PAGE_SUCCESS';
export const CHANGE_PRIZE_PAGE_ERROR = 'CHANGE_PRIZE_PAGE_ERROR';

export const CHANGE_PRIZE_ROW_PER_PAGE = 'CHANGE_PRIZE_ROW_PER_PAGE';
export const CHANGE_PRIZE_ROW_PER_PAGE_SUCCESS =
  'CHANGE_PRIZE_ROW_PER_PAGE_SUCCESS';
export const CHANGE_PRIZE_ROW_PER_PAGE_ERROR =
  'CHANGE_PRIZE_ROW_PER_PAGE_ERROR';

export const SELECT_PRIZE = 'SELECT_PRIZE';
export const SELECT_ALL_PRIZES = 'SELECT_ALL_PRIZES';
export const DESELECT_ALL_PRIZES = 'DESELECT_ALL_PRIZES';
export const UPDATE_PRIZE = 'UPDATE_PRIZE';
export const UPDATE_PRIZE_SUCCESS = 'UPDATE_PRIZE_SUCCESS';
export const UPDATE_PRIZE_ERROR = 'UPDATE_PRIZE_ERROR';
export const CREATE_PRIZE = 'CREATE_PRIZE';
export const CREATE_PRIZE_SUCCESS = 'CREATE_PRIZE_SUCCESS';
export const CREATE_PRIZE_ERROR = 'CREATE_PRIZE_ERROR';
export const BEGIN_AJAX_CALL = 'BEGIN_AJAX_CALL';
export const AJAX_CALL_ERROR = 'AJAX_CALL_ERROR';
export const SHOW_NOTIFICATION_MESSAGE = 'SHOW_NOTIFICATION_MESSAGE';
export const CLOSE_NOTIFICATION_MESSAGE = 'CLOSE_NOTIFICATION_MESSAGE';
export const DELETE_PRIZE = 'DELETE_PRIZE';
export const DELETE_PRIZE_SUCCESS = 'DELETE_PRIZE_SUCCESS';
export const DELETE_PRIZE_ERROR = 'DELETE_PRIZE_ERROR';
export const OPEN_SIDENAV = 'OPEN_SIDENAV';
export const CLOSE_SIDENAV = 'CLOSE_SIDENAV';
export const OPEN_ALERT_DIALOG = 'OPEN_ALERT_DIALOG';
export const CLOSE_ALERT_DIALOG = 'CLOSE_ALERT_DIALOG';
